...
export class AppController {
  ...
  async login(@Body() loginCredentials: LoginDto): Promise<{ accessToken: string }> {
  ...
  @Get('pizza/:id')
  async getPizza(@Headers('authorization') auth: string, @Param('id', ParseIntPipe) id: number): Promise<Pizza> {
    checkAuth(auth);
    return this.appService.getPizza(id);
  }

  @Post('pizza')
  async createPizza(@Headers('authorization') auth: string, @Body(ValidationPipe) pizzaDto: PizzaDto): Promise<number> {
    checkAuth(auth);
    return this.appService.createPizza(pizzaDto);
  }

  @Put('pizza')
  async updatePizza(@Headers('authorization') auth: string, @Body(ValidationPipe) pizzaDto: PizzaDto): Promise<number> {
    checkAuth(auth);
